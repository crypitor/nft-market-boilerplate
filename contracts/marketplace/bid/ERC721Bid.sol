...
contract ERC721Bid is OwnableUpgradeable, PausableUpgradeable, ERC721BidStorage, MinimalForwarderUpgradeable, IERC721Receiver, ERC2771ContextUpgradeable {
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        if (ownerCutPerMillion > 0) {
            ...
            require(
                integratedERC20.transferFrom(bidder, owner(), saleShareAmount),
                "Transfering the cut to the bid contract owner failed"
            );
        }

        // Transfer erc20 token from bidder to seller
        require(
            integratedERC20.transferFrom(bidder, _from, price.sub(saleShareAmount)),
            "Transfering erc20 token to owner failed"
        );
    ...
    * to use bidder erc20 token on his belhalf
    ...
    */
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(
            integratedERC20.balanceOf(_bidder) >= _amount,
            "Insufficient funds"
        );
        require(
            integratedERC20.allowance(_bidder, address(this)) >= _amount,
