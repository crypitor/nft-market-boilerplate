...
import NFTCore from '../../build/contracts/NFTCore.json';
import MetaverseMarket from '../../build/contracts/MetaverseMarket.json';
import Creator from '../../build/contracts/NFTCreator.json';
import Evolver from '../../build/contracts/NFTEvolver.json';
...
enum ContractName {
  MetaverseMarket = 'MetaverseMarket',
  NFTCore = 'NFTCore',
}

const contractNameToProxy: Record<string, ContractName> = {
  NFTCore: ContractName.NFTCore,
  MetaverseMarket: ContractName.MetaverseMarket
}
...
class Ethereum {
  ...
  contractAddresses: Record<ContractName, Array<string>>
  // startBlocks: Record<ContractName, number>s
  ...
  async fetchContracts() {
    const contractsByNetwork = contracts;
    this.contractAddresses = contractsByNetwork[this.network]
  }

  getAddress(contractName: string) {
    return (
      this.contractAddresses[this.getProxyContractName(contractName)][0] ||
  ...
  getStartBlock(contractName: string) {
    return (
      this.contractAddresses[this.getProxyContractName(contractName)][1] ||
